...
public interface IGtfsFlex {
    ...
    @Operation(summary = "returns a gtfs_flex file", description = "returns a specific gtfs_flex file identified by the tdei_record_id", security = {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "A file with the specified tdei_record_id was not found. Use the /api/v1/gtfs-flex endpoints to find valid file ids.", content = @Content(mediaType = "application/json")),
    ...
    @Operation(summary = "List flex files meeting specified criteria.", description = "This endpoint returns a json list of all gtfs flex files stored in the TDEI system that meet the specified criteria. Criteria that can be specified include: polygon (bounding box), minimum confidence level, flex version, date time and organization id.\n" +
            "\n" +
            "This endpoint can be used by an application developer to obtain a list of gtfs flex files in the TDEI system meeting the specified criteria. This endpoint returns a list of file-metadata including the uris of the file, which can be used to fetch the files themselves.", security = {
            @SecurityRequirement(name = "ApiKey"), @SecurityRequirement(name = "AuthorizationToken")}, tags = {"GTFS-Flex"})
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successful response - returns an array of `GtfsFlexDownload` entities.", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = GtfsFlexDownload.class)))),
    ...
    ResponseEntity<List<GtfsFlexDownload>> listFlexFiles(Principal principal,
                                                         ...
                                                         @Parameter(in = ParameterIn.QUERY, example = "[23.5, 34.65, 89.50, 65.98]",
...
//                                                                     @Valid @RequestParam(value = "confidence_level", required = false) Integer confidenceLevel,
                                                         @Parameter(in = ParameterIn.QUERY,
                                                                 description = "version name of the flex schema version that the application requests. list of versions can be found with /api/v1/gtfs-flex/versions.", schema = @Schema())
    ...
    @RequestMapping(value = "versions",
            produces = {"application/json"},
            method = RequestMethod.GET)
    ResponseEntity<VersionList> listFlexVersions(Principal principal, HttpServletRequest req) throws MalformedURLException;
    ...
    ResponseEntity<List<GtfsFlexServiceModel>> listFlexServices(Principal principal, HttpServletRequest httpServletRequest, @Parameter(in = ParameterIn.QUERY, description = "A tdei-assigned id for an organization. org_ids can be retrieved using the path /api/v1/organizations.",
            schema = @Schema()) @Valid @RequestParam(value = "tdei_org_id", required = false) Optional<String> tdei_org_id,
                                                                @Parameter(in = ParameterIn.QUERY, description = "Integer, defaults to 1.", schema = @Schema()) @Valid @RequestParam(value = "page_no", required = false, defaultValue = "1") Integer pageNo,
                                                                @Parameter(in = ParameterIn.QUERY, description = "page size. integer, between 1 to 50, defaults to 10.",
